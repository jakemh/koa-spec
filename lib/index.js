...
const MODULES_DEFAULT_DIRECTORY = './modules';
...
function createRouter(spec, options) {
  ...
  function formatRoute(route) {
    ...
    return route.replace(/{[^/]*}/g, function replacer(match) {
    ...
    _.forOwn(routes, function(methods, route) {
      ...
      _.forOwn(methods, function(methodInfo, method) {
        ...
        if (router[method]) {
          ...
          const middlewarePluginList = spec.resolved['middleware-plugins'];
          const _middlewarePlugins = middlewarePlugins(spec, method, originalRoute, route, methodInfo.parameters, options);

          _middlewarePlugins.forEach(function(plugin) {
            router.use(route, plugin);
          });
...
function middlewarePlugins(spec, method, originalRoute, route, parameterDefinitions, options) {
  const plugins = spec.resolved['middleware-plugins'] || [];

  return plugins.map(function(plugin) {

    return genericMiddleware(plugin, spec, method, originalRoute, route, parameterDefinitions, options);
  }, []);
}

function genericMiddleware(plugin, spec, method, originalRoute, route, parameterDefinitions, options) {
  return function (ctx, next) {
    const pluginArgs = spec.resolved.paths[originalRoute][method][plugin.arguments];

    try {
      const middlewarePluginDirectory = (options && !_.isUndefined(options.middlewarePluginDirectory)) ? options.middlewarePluginDirectory : path.join(path.resolve(), MODULES_DEFAULT_DIRECTORY);
      require(path.join(middlewarePluginDirectory, plugin.module)).call(this, pluginArgs);
      return next();
...
function createRequestValidator(spec, method, route, parameterDefinitions) {
  ...
  return function validateRequest(ctx, next) {
    try {
